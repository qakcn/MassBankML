...
## Files
dataset_casmi_file = input_path / "dataset.casmi.pkl"
dataset_no_casmi_file = input_path / "dataset.no_casmi.pkl"
element_list_file = input_path / "element.nopd.pkl"

dataset_casmi_prepared_file = intermediate_path / "dataset.casmi.prepared.pkl"
dataset_no_casmi_prepared_file = intermediate_path / "dataset.no_casmi.prepared.pkl"

dataset_folds_file = intermediate_path / "dataset.no_casmi.folds.pkl"
...
if dataset_casmi_prepared_file.is_file() and dataset_no_casmi_prepared_file.is_file() and not continue_flag:
    TS("Reading prepared data sets...").p()
    # dataset_folds = torch.load(dataset_folds_file)
    dataset_no_casmi = pd.read_pickle(dataset_no_casmi_prepared_file)
...
if not is_dataset_prepared:
    ...
    dataset_casmi = pd.read_pickle(dataset_casmi_file)
    dataset_no_casmi = pd.read_pickle(dataset_no_casmi_file)
    TS("Done.").green().p()

    tqdm.pandas(desc="Preparing data set within CASMI", total = dataset_casmi.shape[0], ncols=100)
    dataset_casmi = dataset_casmi.progress_apply(prepare_row, axis=1, element_list=element_list)

    tqdm.pandas(desc="Preparing data set without CASMI", total = dataset_no_casmi.shape[0], ncols=100)
    dataset_no_casmi = dataset_no_casmi.progress_apply(prepare_row, axis=1, element_list=element_list)

    TS("Saving prepared data sets...").ip()
    dataset_casmi.to_pickle(dataset_casmi_prepared_file)
    dataset_no_casmi.to_pickle(dataset_no_casmi_prepared_file)
    TS("Done.").green().p()

    # TS("Splitting data sets: ").ip()
    # fold_sizes = [len(dataset_no_casmi) // 10] * 9
    # fold_sizes.append(len(dataset_no_casmi) - sum(fold_sizes))
    # dataset_no_casmi = MassbankMLDataset(dataset_no_casmi)
    # dataset_folds = random_split(dataset_no_casmi, fold_sizes)
    # TS("Done.").green().p()

    # TS("Saving folded data sets...").ip()
    # torch.save(dataset_folds, dataset_folds_file)
    # TS("Done.").green().p()
...
if is_dataset_prepared:
    if continue_flag:
        ...
        (train_output_path/"limit 9000_1500").touch()
    ...
    if not continue_flag:
        ...
        # train_ds_list = []
        # for i in range(10):
        #     if i == configurations["learning"]["kfold"]:
        #         valid_dataset = dataset_folds[i]
        #     else:
        #         train_ds_list.append(dataset_folds[i])
        # train_dataset = ConcatDataset(train_ds_list)
        dataset_no_casmi = MassbankMLDataset(dataset_no_casmi)
        train_size = 9000
        valid_size = 1500
        void_size = len(dataset_no_casmi) - train_size - valid_size
        train_dataset, valid_dataset, _ = random_split(dataset_no_casmi, [train_size, valid_size, void_size])
    ...
    train_loader = DataLoader(train_dataset, batch_size=configurations["learning"]["batch_size"], shuffle=True, num_workers=8)
    valid_loader = DataLoader(valid_dataset, batch_size=configurations["learning"]["batch_size"], shuffle=True, num_workers=8)
    TS("Done.").green().p()
    ...
    accuracy=tm.Accuracy(task="binary", average="macro").to(device)
    precision=tm.Precision(task="binary", average="macro").to(device)
    auc=tm.AUROC(task="binary", average="macro").to(device)
    recall=tm.Recall(task="binary", average="macro").to(device)
    f1=tm.F1Score(task="binary", average="macro").to(device)
    TS("Done.").green().p()
